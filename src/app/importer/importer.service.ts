...
export async function importDeputies() {
  const data = loadFolder(
    path.join("data", "AMO10_deputes_actifs_mandats_actifs_organes.json")
  );
...
export async function importAmendments() {
  ...
  // 🔥 1. précharger tous les laws existants
  const laws = await prisma.law.findMany({
    select: { externalId: true },
  });
  if (!laws || laws.length === 0) {
    console.warn("importAmendments: no laws found, skipping amendments import");
    return;
  }
  const lawSet = new Set(laws.map((l: { externalId: string }) => l.externalId));
  ...
  for (const raw of data) {
    ...
    // 🔥 2. validation FK AVANT upsert
    const safeLawId = lawId && lawSet.has(lawId) ? lawId : null;

    if (!safeLawId) {
      console.warn("missing law, skipping amendment:", a.uid);
      skipped++;
      continue;
    }
    ...
    try {
      ...
      console.error("upsert failed:", a.uid, e);
      skipped++;
    }
  }
